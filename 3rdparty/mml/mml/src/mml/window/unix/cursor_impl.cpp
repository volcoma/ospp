...
////////////////////////////////////////////////////////////
bool cursor_impl::load_from_pixels(const std::uint8_t* pixels, std::array<std::uint32_t, 2> size,
								   std::array<std::uint32_t, 2> hotspot)
{
	...
	for(std::size_t j = 0; j < size[1]; ++j)
	{
		for(std::size_t i = 0; i < size[0]; ++i)
		{
			...
			int intensity =
				pixels[pixelIndex * 4 + 0] + pixels[pixelIndex * 4 + 1] + pixels[pixelIndex * 4 + 2];
	...
	Pixmap maskPixmap =
		XCreateBitmapFromData(display_, XDefaultRootWindow(display_), (char*)&mask[0], size[0], size[1]);
	Pixmap dataPixmap =
		XCreateBitmapFromData(display_, XDefaultRootWindow(display_), (char*)&data[0], size[0], size[1]);
	...
	cursor_ = XCreatePixmapCursor(display_, dataPixmap, maskPixmap, &fg, &bg, hotspot[0], hotspot[1]);
...
bool cursor_impl::load_from_system(cursor::type type)
{
	...
	switch(type)
	{
		...
		case cursor::hand:
			shape = XC_hand2;
			break;
		case cursor::size_horizontal:
			shape = XC_sb_h_double_arrow;
			break;
		case cursor::size_vertical:
			shape = XC_sb_v_double_arrow;
			break;
		case cursor::size_top_left_bottom_right:
			shape = XC_fleur;
			break;
		case cursor::size_bottom_left_top_right:
			shape = XC_fleur;
			break;
