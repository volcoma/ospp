...
bool is_joystick(udev_device* udevDevice)
{
	...
	if(idClass)
	{
		...
		if(std::strstr(idClass, "accelerometer") || std::strstr(idClass, "key") ||
		   std::strstr(idClass, "keyboard") || std::strstr(idClass, "mouse") ||
		   std::strstr(idClass, "tablet") || std::strstr(idClass, "touchpad") ||
...
const char* get_usb_attribute(udev_device* udevDevice, const std::string& attributeName)
{
	udev_device* udevDeviceParent =
		udev_device_get_parent_with_subsystem_devtype(udevDevice, "usb", "usb_device");
...
unsigned int get_joystick_vendor_id(unsigned int index)
{
	...
	udev_device* udevDevice =
		udev_device_new_from_syspath(udevContext, joystickList[index].system_path.c_str());
...
void joystick_impl::initialize()
{
	...
	if(!udevMonitor)
	{
		err() << "Failed to create udev monitor, joystick connections and disconnections won't be notified"
			  << std::endl;
	}
	else
	{
		...
		if(error < 0)
		{
			err() << "Failed to add udev monitor filter, joystick connections and disconnections won't be "
					 "notified: "
				  << error << std::endl;
		...
		else
		{
			...
			if(error < 0)
			{
				err() << "Failed to enable udev monitor, joystick connections and disconnections won't be "
						 "notified: "
					  << error << std::endl;
		...
		switch(joyState.type & ~JS_EVENT_INIT)
		{
			...
			case JS_EVENT_AXIS:
			{
				...
				if(joyState.number < ABS_MAX + 1)
				{
					...
					{
						case ABS_X:
							state_.axes[joystick::X] = value;
							break;
						case ABS_Y:
							state_.axes[joystick::Y] = value;
							break;
						case ABS_Z:
						case ABS_THROTTLE:
							state_.axes[joystick::Z] = value;
							break;
						case ABS_RZ:
						case ABS_RUDDER:
							state_.axes[joystick::R] = value;
							break;
						case ABS_RX:
							state_.axes[joystick::U] = value;
							break;
						case ABS_RY:
							state_.axes[joystick::V] = value;
							break;
						case ABS_HAT0X:
							state_.axes[joystick::PovX] = value;
							break;
						case ABS_HAT0Y:
							state_.axes[joystick::PovY] = value;
							break;
						default:
							break;
					}
